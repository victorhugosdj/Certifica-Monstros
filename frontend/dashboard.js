/*
  dashboard-unified.js - Dashboard Unificado: Útil + Agradável
  Combina simplicidade (métricas, recomendações, questões desafiadoras) com visual avançado (gráficos, exames oficiais, ranking expandido).
  
  COMPONENTS:
  1. Ranking (TOP 3 ou completo, com progresso público)
  2. Métricas gerais (simples + gráficos)
  3. Gráficos (Pizza, Radar, Barras - opcionais)
  4. Módulos com progresso
  5. Recomendações personalizadas
  6. Questões desafiadoras
  7. Exames oficiais (opcionais)
*/
...
/**
 * Renderizar ranking de usuários (TOP 3) - versão simples se não expandido
 */
...
async function initDashboard() {
  ...
  try {
    ...
    // Renderizar todos os componentes em ordem lógica
    renderMetrics(stats);
    if (ranking && ranking.length > 3) {
      renderRanking(CURRENT_USER.id, ranking);  // Ranking expandido com progresso público
    } else {
      renderRankingTable(CURRENT_USER.id, ranking);  // Ranking simples TOP 3
    }
    renderChartAcertosErros(stats);  // Gráfico opcional
    ...
    if (remoteMetrics && remoteMetrics.grafico_radar) {
      renderRadarChart(remoteMetrics.grafico_radar);
    }
    
    // Exames oficiais opcionais
    renderOfficialExams();
